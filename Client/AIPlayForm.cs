...
namespace Client
{
    public partial class AIPlayForm : Form
    {
        ...
        // Alpha-Beta Pruning 기법을 적용했을때 구해지는 표적의 수
        // 오목 인공지능 입장에서 보았을때 두면 가장 좋을 수
        private int targetX, targetY;

        // 내다보고자 하는 수 Ex) 한수를 앞서서 본다↓↓↓↓ㄴ
        // limit = 0 이면 자기자신, limit = 1 이면 한수 앞서서 본다
        private int limit = 1;   
        ...
        // 평가함수
        // ※ 평가함수를 어떻게 만드냐에 따라서 인공지능의 성능이 달라진다!!!
        private int evaluate(Horse horse)     
        {
            ...
            sum += half2 * 300;
            sum += close2 * 50;
            return sum;  // sum 값 자체가 평가함수의 결과값이 된다.
        }

        // ※ AlphaBetaPruning 알고리즘 함수 ※
        // Minimax 알고리즘에서 Max 역할 ---> int alpha, Min 역할 ---> int beta
        int AlphaBetaPruning(int level, int alpha, int beta)
        {
            // 만약에 현재 최대 깊이인 limit에 도달한 경우
            if (level == limit)
            {
                // AI의 평가 가치에서 플레이어의 평가 가치를 뺀 수를 반환(평가함수)
            ...
            if (level % 2 == 0)
            {
                ...
                int find = 0;
                // 전체 바둑판을 모두 탐색하면서 DFS(깊이우선탐색)을/를 진행
                for (int i = 0; i < edgeCount; i++)
                {
                    for (int j = 0; j < edgeCount; j++)
                    {
